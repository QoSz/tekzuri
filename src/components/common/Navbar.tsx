...
export function Navbar() {
  return (
    <nav className="sticky top-0 z-50 bg-background/60 backdrop-blur-md border-b border-white/10">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-5 py-2 text-sm font-medium text-muted-light hover:text-foreground transition-colors duration-200 relative group"
              >
                {link.label}
